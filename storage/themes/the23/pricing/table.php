		...
		<tbody>
			...
			<?php if($features = \Helpers\App::features()): ?>
				<?php foreach($features as $slug => $feature): ?>
					<tr>
						<td class="py-3 text-start fw-bold"><?php echo $feature['name'] ?> <i class="align-middle fa fa-question-circle opacity-50 text-muted small ms-2" data-bs-toggle="tooltip" title="<?php echo $feature['description'] ?>"></i></td>
						<?php foreach($plans as $plan): ?>
							<td>
								...
								<?php else: ?>
									<?php if($slug == 'apirate'): ?>
                                        <?php $rate = appConfig('app.throttle') ?>
                                        <span class="fw-bolder"><?php ee('{x}/min', null, ['x' => $rate[0]]) ?> </span>
                                    <?php else: ?>
